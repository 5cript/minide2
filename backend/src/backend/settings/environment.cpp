...
namespace
{
    struct range_iless
    {
        ...
        bool operator()(InputRange1 const& r1, InputRange2 const& r2) const
        {
            ...
            return std::lexicographical_compare(begin(ir1), end(ir1), begin(ir2), end(ir2), boost::is_iless{});
        }
    };
...
namespace SettingParts
{
    ...
    {
        j = json{{"path", env.path}, {"variables", env.variables}, {"inherits", env.inherits}};
    }
    //---------------------------------------------------------------------------------------------------------------------
