...
namespace Backend::Server::Api
{
    ...
    void Workspace::doSubscribe()
    {
        ...
        */
        subscribe("/api/workspace/loadFile", [this](json const& j) {
            loadFile(j["ref"].get<int>(), j["payload"]["path"].get<std::string>());
        });
        ...
        */
        subscribe("/api/workspace/setActiveProject", [this](json const& j) {
            setActiveProject(j["ref"].get<int>(), j["payload"]["path"].get<std::string>());
        });
        ...
        */
        subscribe("/api/workspace/deleteFile", [this](json const& j) {
            deleteFile(j["ref"].get<int>(), j["payload"]["path"].get<std::string>());
        });
    }
    //---------------------------------------------------------------------------------------------------------------------
    void Workspace::open(int ref, sfs::path const& root)
    {
        ...
        root_ = root;
        sess->writeJson({{"ref", ref}, {"directory", dir}});
    }
    //---------------------------------------------------------------------------------------------------------------------
    ...
    void Workspace::setActiveProject(int ref, sfs::path const& path)
    {
        ...
        activeProject_ = path;
        sess->writeJson({{"ref", ref}, {"activeProject", path}});
    }
    //---------------------------------------------------------------------------------------------------------------------
    void Workspace::deleteFile(int ref, sfs::path const& path)
    {
        ...
        sess->writeJson({{"ref", ref}, {"deleted", path}});
    }
    //---------------------------------------------------------------------------------------------------------------------
    sfs::path Workspace::getRoot() const
    {
        return root_;
    }
    //---------------------------------------------------------------------------------------------------------------------
    sfs::path Workspace::getActiveProject() const
    {
        return activeProject_;
    }
    //---------------------------------------------------------------------------------------------------------------------
    std::tuple<sfs::path, sfs::path, bool, bool>
...
#define TEST_SPECIAL_DIR(X) \
    if (specialDir.empty() && sfs::exists(X)) \
        ...
        return {specialDirPartner.generic_string(), samePlacePartner.generic_string(), specialExists, anyFound};
    }
    //---------------------------------------------------------------------------------------------------------------------
    std::tuple<std::string, std::string, json>
    Workspace::verifyPath(std::string path, sfs::path const& root, bool mustExist, bool enforceWorkspaceRelative)
    {
        if (path.empty())
        ...
        else if ((path.length() > rootFilename.length()) && path.substr(0, rootFilename.length()) == rootFilename)
        {
            ...
            if (!relative && enforceWorkspaceRelative)
                return {
                    "Path is not within workspace, and thats forbidden."s,
                    path,
                    json::object({"pathNotInWorkspace", true})};
        ...
        if (mustExist && !sfs::exists(resultPath))
            return {"Path does not exist.", path, json{{"fileNotFound", true}, {"path", path}}};
